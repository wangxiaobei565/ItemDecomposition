...
mkdir -p output/ml1m/agents/search/reqsuper/
...
for MAX_STEP in 20 
do
    for BEHAVIOR_LR in 0.000001 0.000003 0.00001 0.00003 0.0001 0.0003 0.001 0.003
    do
        ...
        do
            ...
            do
                for GAMMA in 0.9 
                do
                    mkdir -p ${output_path}agents/search/reqsuper/Super_${SCORER}_behavior${BEHAVIOR_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/

                    python train_ac.py\
                        --env_class ML1MEnvironment_GPU\
                        --policy_class OneStagePolicy_with_${SCORER}\
                        --critic_class HACCritic\
                        --agent_class Supervised\
                        ...
                        --save_path ${output_path}agents/search/reqsuper/Super_${SCORER}_behavior${BEHAVIOR_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/model\
                        ...
                        --check_episode 10\
                        --topk_rate ${TOPK}\
                        > ${output_path}agents/search/reqsuper/Super_${SCORER}_behavior${BEHAVIOR_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/log
                done
            done
        done
    done
