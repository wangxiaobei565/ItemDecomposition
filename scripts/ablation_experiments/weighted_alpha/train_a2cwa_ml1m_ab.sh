...
mkdir -p output/ml1m/agents/alpha/itemA2C_W/
...
do
    ...
    do
        for ALPHA in 0 0.2 0.4 0.6 0.8 0.999 1.2 1.4 1.6 1.8 2.0
        do
            for SEED in 11 13 17 19 23
            do
                for ACTOR_LR in 0.00003
                do
                    mkdir -p ${output_path}agents/alpha/itemA2C_W/ml1m_wa_alpha${ALPHA}_${SCORER}_actor${ACTOR_LR}_critic${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/
                    ...
                    python train_ac.py\
                        ...
                        --save_path ${output_path}agents/alpha/itemA2C_W/ml1m_wa_alpha${ALPHA}_${SCORER}_actor${ACTOR_LR}_critic${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/model\
                        --alpha ${ALPHA}\
                        ...
                        --check_episode 10\
                        --topk_rate ${TOPK}\
                        > ${output_path}agents/alpha/itemA2C_W/ml1m_wa_alpha${ALPHA}_${SCORER}_actor${ACTOR_LR}_critic${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/log
                done
            done
        done
    done
