...
mkdir -p output/ml1m/agents/itemA2C_weight/
...
for WEIGHT_LR in 1e-8 
do
    for SLATE in 1 2 4 6 8 
    do
        for CRITIC_LR in 3e-6
        do
            for SEED in 11 13 17 19 23
            do
                for ACTOR_LR in 3e-5
                do
                    mkdir -p ${output_path}agents/itemA2C_weight/weight_${SCORER}_actor${ACTOR_LR}_critic${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/
                    ...
                    python train_ac_weight.py\
                        ...
                        --save_path ${output_path}agents/itemA2C_weight/weight_${SCORER}_actor${ACTOR_LR}_critic${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/model\
                        ...
                        --check_episode 10\
                        --topk_rate ${TOPK}\
                        > ${output_path}agents/itemA2C_weight/weight_${SCORER}_actor${ACTOR_LR}_critic${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/log
                done
            done
        done
    done
