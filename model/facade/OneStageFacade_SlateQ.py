...
class OneStageFacade_SlateQ():
    ...
    def __init__(self, args, environment, critic):
        super().__init__()
...
#             "next_history_features": torch.zeros(self.buffer_size, self.env.reader.max_seq_len, self.env.reader.item_vec_size),
            "state_emb": torch.zeros(self.buffer_size, self.critic.state_dim),
...
#         return (np.mean(self.hist_Q) + self.q_laplace_smoothness) / (new_q + self.q_laplace_smoothness)
    ...
    def apply_policy(self, observation, critic_model, epsilon = 0.1, 
                     do_explore = False):
        '''
        @input:
...
#             # (B, L, item_dim)
#             feed_dict['candidate_features'] = feed_dict['candidate_features']
#             # (B, L)
#             feed_dict['candidate_ids'] = feed_dict['candidate_ids']
#             batch_wise = True
#         else:
#             # (1,L,item_dim)
#             feed_dict['candidate_features'] = self.candidate_features.unsqueeze(0)
#             # (L,)
#             feed_dict['candidate_ids'] = self.candidate_iids
#             batch_wise = False
        feed_dict['candidate_features'] = self.candidate_features.unsqueeze(0)
        # (L,)
        feed_dict['candidate_ids'] = self.candidate_iids
...
#             action_emb = out_dict['action_emb']
            ...
            if np.random.rand() < epsilon:
                indices = torch.randint(0, len(self.candidate_iids), 
                                        (candidate_prob.shape[0], self.slate_size)).to('cuda:0')
#                 action_emb = torch.clamp(torch.rand_like(action_emb)*self.noise_var, -1, 1)
            else:
                action, indices = utils.sample_categorical_action(candidate_prob, 
...
#             action, indices = utils.sample_categorical_action(candidate_prob, out_dict['candidate_ids'], 
#                                                               self.slate_size, with_replacement = False, 
#                                                               batch_wise = batch_wise, return_idx = True)
    
                
        else:
            _, indices = torch.topk(out_dict['q_all'], k = self.slate_size, dim = 1)
...
#         import pdb
#         pdb.set_trace()
        out_dict['q'] = torch.gather(out_dict['q_all'], 1, indices)
            # topk action
    ...
    def sample_buffer(self, batch_size):
        ...
        U, H, N, S, A, R, F, D = self.read_buffer(indices)
        observation = {"user_profile": U, "history_features": H}
        policy_output = {"state_emb": S, "action": A}
