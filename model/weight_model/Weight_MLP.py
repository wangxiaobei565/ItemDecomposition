...
#         self.state_encoder = policy.state_encoder
        self.net = DNN(self.state_dim *2 + self.f_dim*self.ls_size + self.ls_size, args.critic_hidden_dims1, self.ls_size, 
                       dropout_rate = args.critic_dropout_rate1, do_batch_norm = True)
...
#         state_emb = self.state_encoder(feed_dict)['state_emb'].view(-1, self.state_dim)
        ls_emb_ = curr_feed_dic['list_emb'].detach()
        next_state_emb = next_feed_dic['state_emb'].detach()
        ls_emb = torch.flatten(ls_emb_, start_dim=1)
        
        weight = self.net(torch.cat([state_emb, ls_emb, next_state_emb,reward], dim = 1))
